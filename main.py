...
app.config['UPLOAD_FOLDER'] = 'static/proof'  # Folder to store songs


app.config['UPLOAD_FOLDER1'] = 'static/vehicle'
        ...
        try:
            ...
            if file and allowed_file(file.filename):
                ...
                if file1 and allowed_file(file1.filename):
                    ...
                    image_path = 'static/vehicle/' + filename
                    file1.save(image_path)  # ✅ Corrected

                    # ✅ Check if vehicle already exists
                    mycursor = mydb.cursor()
                    mycursor.execute("SELECT count(*) FROM pm_vehicle WHERE veh_no=%s AND username=%s AND images=%s", (veh_no, username, filename))

                    cnt = mycursor.fetchone()[0]

                    if cnt == 0:
                        mycursor.execute("SELECT IFNULL(MAX(id)+1, 1) FROM pm_vehicle")
                        maxid = mycursor.fetchone()[0]

                    sql = """INSERT INTO pm_vehicle(id, veh_no, reg_mobile, reg_name, reg_address, veh_type, veh_name, 
                            veh_color, fuel_type, chassis_no, seats, reg_join, image, images, username, password) 
                            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
                    
                    val = (maxid, veh_no, reg_mobile, reg_name, reg_address, veh_type, veh_name, veh_color, 
                           fuel_type, chassis_no, seats, reg_join, random_filename, filename, username, password)
                ...
                msg = "Invalid file format! Only JPG, JPEG, and PNG allowed."

        except mysql.connector.Error as err:
            mydb.rollback()  # Rollback changes on error
...
def num_search():
    ...
    if request.method == 'POST':
        ...
        if doc_file:
            ...
            doc_file.save('static/vehicle/' + doc_filename)
            doc_path = 'static/vehicle/' + doc_filename

            cursor = mydb.cursor(buffered=True)
            cursor.execute("SELECT * FROM pm_vehicle WHERE images = %s", (doc_filename,))
            da = cursor.fetchone()
            cursor.close()
            license_plate=da[1]
            ...
            for contour in contours:
                if cv2.contourArea(contour) > 1000:
                    ...
                    license_plate1 = pytesseract.image_to_string(roi, config='--psm 6')  # Adjust OCR configuration if needed
                    break
    ...
    if request.method=='POST':       
        ...
        if 'profile' in request.files:
            ...
            if profile and allowed_file(profile.filename):
                ...
                profile_path = 'static/driver/' + filename
                profile.save(profile_path)

                mycursor = mydb.cursor()
                mycursor.execute("SELECT count(*) FROM pm_driver where mobile=%s",(mobile, ))
                cnt = mycursor.fetchone()[0]
                if cnt==0:
                    ...
                    if maxid is None:
                        maxid=1
                    sql = "INSERT INTO pm_driver(id, name, address, mobile, email, profile, date_join, veh_no, owner_username) VALUES (%s, %s, %s, %s, %s, %s, %s, %s,%s)"
                    val = (maxid, name, address, mobile, email, filename, date_join, veh_no, owner_username)
...
def view_status():
    ...
    cursor = mydb.cursor()
    cursor.execute("SELECT * FROM pm_travel where username = %s",(username,))
